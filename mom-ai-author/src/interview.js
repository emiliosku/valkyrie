...
async function generate(id, store) {
  ...
  let result = session.mock ? { text: JSON.stringify(mockQuest(session)), model: 'mock', latencyMs: 0 } : await complete(store, [{ role: 'system', content: generationPrompt(session) }, { role: 'user', content: 'Generate the complete scenario now.' }], { stage: 'generation', accept: generatedFilesValid, timeoutMs: 90000, maxTokens: 4000, temperature: 0.2 });
