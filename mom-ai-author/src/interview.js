...
const QUEST_SCHEMA = 'Use Valkyrie MoM INI syntax exactly. quest.ini must contain [Quest] with format=21, type=MoM, packs=, defaultlanguage=English, defaultmusicon=True, hidden=False, minhero=2, maxhero=5, difficulty=number (0-5 scale), lengthmin=minutes, lengthmax=minutes, and name.English=title. quest.ini must also contain [QuestText] listing Localization.English.txt and [QuestData] listing tiles.ini, events.ini, and tokens.ini. Localization.English.txt must start with ".,English" then define quest.name (display title), quest.synopsys (1-2 sentence teaser shown in quest card, max 100 chars), quest.description (full synopsis), quest.authors_short (e.g. AI Generated), then qst:EventName.text entries for each displayed event. Each [Tile...] in tiles.ini requires side=TileSide..., xposition=number, yposition=number, and is named in an event add=. Each [Token...] requires type=Token..., xposition, yposition, and is added by an event. Optional [Spawn...] sections in spawns.ini require monster=Monster..., xposition, yposition, and an event add=. Events use lowercase display=, buttons=, button1=, event1=, add=, remove=, and operations=. Do not use Id=, Name=, Text=, Orientation=, or uppercase Button1/Event1. End each victory or failure path with operations=$end,=,1.';
...
function mockQuest(session) {
  ...
  return { name: title, files: {
    ...
    'Localization.English.txt': `.,English\nquest.name,${title}\nquest.synopsys,A sealed archive awakens beneath the manor.\nquest.description,A sealed archive awakens beneath the manor.\nquest.authors_short,AI Generated\nqst:EventArchive.text,${title}\n`,
  }};
}
...
async function next(session, store, forceReady = false) {
  ...
  session.model = result.key; session.latencyMs = (session.latencyMs || 0) + (result.latencyMs || 0); return publicSession(session);
}
